...
if len(miss_df) > 0:
    ...
    pd.DataFrame(columns=["run","case","relpath","file_name","basic_file","expected_run_file"]).to_csv(out / "missing_files_vs_basic.csv", index=False)

# standard-only explicit diff list for user ask
std_missing = miss_df[miss_df["run"] == "standard"] if len(miss_df) > 0 else pd.DataFrame(columns=miss_df.columns)
std_missing.to_csv(out / "missing_files_standard_vs_basic.csv", index=False)
...
# Both heatmaps use same color scheme now (plasma)
heat("benchmark", "heatmap_benchmark_only.png", "plasma", "Benchmark only: Case x FileName match (%)")
...
# Missing files summary by run/case
if len(miss_df) > 0:
    miss_summary = miss_df.groupby(["run", "case"]).size().reset_index(name="missing_count").sort_values(["run", "missing_count"], ascending=[True, False])
else:
    miss_summary = pd.DataFrame(columns=["run", "case", "missing_count"])
miss_summary.to_csv(out / "missing_files_summary_by_case.csv", index=False)

summary = []
summary.append("# Dual heatmaps + difference analysis (v2)")
summary.append("")
summary.append("Included files: 0/, constant/, system/.")
summary.append("Included case: damBreakWithObstacle (now included).")
...
for p in [
    ...
    "missing_files_vs_basic.csv",
    "missing_files_standard_vs_basic.csv",
    "missing_files_summary_by_case.csv",
]:
...
summary.append(f"- Total missing (both runs): {len(miss_df)}")
summary.append(f"- Missing in standard: {len(std_missing)}")
